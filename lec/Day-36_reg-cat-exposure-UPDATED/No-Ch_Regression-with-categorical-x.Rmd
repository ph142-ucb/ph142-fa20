...
This shows us that `type` is coded as "chr", which stands for character. This means it isn't yet stored as a factor variable, which is what we want to have for putting it into the analysis. 
...
$$\hat{y_i} = \hat{a} + \hat{b_1}*[\text{i's category == osteopenia}] + \hat{b_2}[\text{i's category == osteoporosis}]$$
...
* The coefficient for "osteopenia" is equal to -138.33. This means that individual's with osteopenia had calcium intakes that are on average 138.33 lower than individuals with normal bone density.
* The coefficient for "osteoporosis" is equal to -223.33. This means that individual's with osteoporosis had calcium intakes that are on average 223.33 lower than individuals with normal bone density.
...
$$\hat{y_i} = \hat{a} + \hat{b_1}*[\text{i's category == normal}] + \hat{b_2}[\text{i's category == osteopenia}]$$
