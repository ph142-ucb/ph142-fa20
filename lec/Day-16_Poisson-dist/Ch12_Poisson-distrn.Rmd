...
* `ppois(q = a lambda = 0.1)` is the probability of `a` events **or less**. Thus `ppois` returns a cumulative probability of the lower tail by default.

* `dpois(q = a lambda = 0.1)` is the probability of **exactly** `a` events. 

So to calculate the probability of 1 events or less we could use ppois() like this:
```{r}
