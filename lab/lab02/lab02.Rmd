...
- `dim()`: Provides the dimensions of our dataset; first number correlates to the number of rows, and the second number correlates to the number of columns/variables.

- `str()`: Returns information about the dataset, including the variables and the types (char, num), and also provides a few examples within the dataset.

- `names()`: Returns the names of the variable in our dataset.

- `head()`: Returns the first 6 rows of our dataset.

**Based on these functions' outputs, who/what are the individuals in this 
dataset? How many of them are there?**

The individuals in our dataset are the countries, and there are 137 countries listed.
...
```{r add-new-variable}
CS_data <- mutate(CS_data_raw, CS_rate_100 = CS_rate * 100)
...
```{r bar-chart-income}
p2 <- ggplot(CS_data, aes(x = Income_Group)) +
  geom_bar()
p2
...
p3 <- ggplot(CS_data, aes(x = Income_Group_order)) +
  geom_bar()
p3
...
Income_Group_order is preferred because it orders our income groups in a way that makes the most sense to us.


**Did you use the argument `stat="identity"` in the code to make this bar chart? Why or why not?**

We did not; we do not have a separate variable that relates to the percentages. Instead, we relied on the counts of each of the categories in 'Income_Group_order.'
...
p4 <- ggplot(CS_data, aes(x = Region)) +
  geom_bar() 
p4
...
p5 <- ggplot(CS_data, aes(x = GDP_2006)) +
  geom_histogram()
p5
...
p6 <- ggplot(CS_data, aes(x = GDP_2006)) +
...
```{r histogram-CS}
p7 <- ggplot(CS_data, aes(x = CS_rate_100)) +
...
```{r your-final-plots}
p8 <- ggplot(CS_data, aes(x = CS_rate_100)) +
  geom_histogram(binwidth = 5, col = "white", fill = "darksalmon") +
  labs(y = "Count of Countries", x = "Cesarean delivery rate (%)", title = "Distribution of cesarean delivery rates across countries")
p8
...
```{r saving-plot}
ggsave("hist-cs.png", plot = p8, device = "png", width = 8, height = 5, units = "in")
